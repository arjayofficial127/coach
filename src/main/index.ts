...
import { runPhaseTwoSmoke } from "./smoke";
...
app.whenReady().then(async () => {
  if (process.argv.includes("--phase2-smoke")) {
    try {
      const rendererRoot = path.join(__dirname, "../renderer");
      const evidence = await runPhaseTwoSmoke(rendererRoot, path.join(__dirname, "preload.cjs"));
