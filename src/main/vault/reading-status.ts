...
import type { ReadingStatus } from "../../shared/contracts";
...
export async function updateReadingStatusAtomically(
  vaultRoot: string,
  relativePath: string,
  expectedId: string,
  ...
  if (
    ...
    throw new Error("Reading status can only update a saved-link Markdown note.");
  }
  ...
  frontmatter = setFrontmatterScalar(frontmatter, "reading_status", status);
  frontmatter = setFrontmatterScalar(
    frontmatter,
    "queued_at",
    status === "queued" ? changedAt : status === "read" ? record.queuedAt : "",
  );
  frontmatter = setFrontmatterScalar(frontmatter, "read_at", status === "read" ? changedAt : "");
  ...
  try {
    ...
    if (current.size !== before.size || current.mtimeMs !== before.mtimeMs) {
      throw new Error("The saved-link note was edited while its reading status was changing.");
    }
    await assertNoLinks(canonicalRoot, target);
